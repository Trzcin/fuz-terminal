  ...
  const sanitized = params.reduce<string>((result, param, idx) => {
    ...
    let parsedParam = arrayParam.reduce<string>((acc, p) => {
      if (p == null || p === false) {
        ...
        return (
          acc +
          p.toString().replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;")
        );
      }
    }, "");
...
export function css(template: TemplateStringsArray): CSSExpression {
  return new CSSExpression(template.join(""));
}
